...
int ll_deleteLinkedList(LinkedList* this)
{
    ...
    if(this != NULL)
    {
    	if(!ll_clear(this))//entro cuando clear me dio 0 (es decir, funcionó)
    	{
    		free(this);
    		returnAux = 0;
    	}
...
int ll_indexOf(LinkedList* this, void* pElement)
{
    ...
    if(this !=NULL)
    {
    	...
    	for(int i=0 ; i<tam;i++)
    	{
    		...
    		{
    			returnAux = i;
    			break;//porque me pide sólo la primer ocurrencia
    		}
    	}
    }
...
LinkedList* ll_map(LinkedList* this,int* (pFunc)(void*))
{
    ...
    void* pElement = NULL;
    LinkedList* map = ll_newLinkedList();
    int funcion;
    ...
    if(this!=NULL && pFunc!=NULL)
    {
        ...
        for (int i = 0; i < tam; i++)
        {
            ...
            funcion = pFunc(pElement);

            if(pElement!=NULL)
            {
                if(funcion == 0)//si sale todo bien...
                {
            	ll_add(map, pElement);//añade
                }
                else //si no sale todo bien...
                {
                	map = NULL; //apunta a NULL la nueva linked list
	...
	if(this != NULL)
	{
		...
		if(listaFiltro != NULL)
		{
			...
			for(int i =0;i<tam;i++)
			{
				...
				if(pFunc(aux) == 0)
				{
					if(ll_add(listaFiltro, aux) == -1) //si no puede crearla
					{
						ll_deleteLinkedList(listaFiltro); //elimina la lista de filtro
						listaFiltro = NULL; //apunta a NULL la lista de filtro
					}
				}
			}
		}
	}
	return listaFiltro; //retorna la nueva lista de filtro creada o si no pudo crearla retorna "NULL"
}
